...
- Monitor: **`A` applies an accepted loop's diff to your working tree, then
  ...
  runs it for you, behind a y/n confirm, and — since applying is shipping —
  clears the finished loop from the rail (the logbook keeps a line). This is
  the only place the monitor touches your checkout, and it is the weakest
  possible touch — `git apply` to the working tree, **never a commit, push, or
  merge** — so the patch lands as an uncommitted change you review, commit, and
  ship. A patch that doesn't fit leaves both your tree and the loop untouched
  and says so.
